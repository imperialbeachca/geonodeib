...
## Quick Docker Start
  ...
  ```bash
    ...
    source ~/.venvs/geonodeib/bin/activate
...
**NOTE**: *You can call your geonode project whatever you like **except 'geonode'**. Follow the naming conventions for python packages (generally lower case with underscores (``_``). In the examples below, replace ``geonodeib`` with whatever you would like to name your project.*
...
1. Generate the project
    ...
    mkvirtualenv --python=/usr/bin/python3 geonodeib
    pip install Django==3.2.16

    django-admin startproject --template=./geonode-project -e py,sh,md,rst,json,yml,ini,env,sample,properties -n monitoring-cron -n Dockerfile geonodeib
...
```bash
SOURCE_URL=$SOURCE_URL TARGET_URL=$TARGET_URL ./geonodeib/br/backup.sh $BKP_FOLDER_NAME
```
...
```bash
docker exec -it django4geonodeib sh -c 'SOURCE_URL=$SOURCE_URL TARGET_URL=$TARGET_URL ./geonodeib/br/backup.sh $BKP_FOLDER_NAME'
```

### Run a Restore

```bash
SOURCE_URL=$SOURCE_URL TARGET_URL=$TARGET_URL ./geonodeib/br/restore.sh $BKP_FOLDER_NAME
```
...
```bash
docker exec -it django4geonodeib sh -c 'SOURCE_URL=$SOURCE_URL TARGET_URL=$TARGET_URL ./geonodeib/br/restore.sh $BKP_FOLDER_NAME'
```
